// filepath: src\config\adapters\ResourceStorageDataAdapter.java
...
public class ResourceStorageDataAdapter implements JsonDeserializer<ResourceStorageData> {
    @Override
    public ResourceStorageData deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
            throws JsonParseException {
        JsonObject jsonObject = json.getAsJsonObject();
        ResourceStorageData data = new ResourceStorageData();

        if (jsonObject.has("capacity")) {
            data.setBaseCapacity(jsonObject.get("capacity").getAsInt());
        }
        if (jsonObject.has("type")) {
            String resourceStr = jsonObject.get("type").getAsString().toUpperCase();
            try {
                data.setResourceStore(ResourceType.valueOf(resourceStr));
            } catch (IllegalArgumentException e) {
                data.setResourceStore(ResourceType.NONE);
            }
        }
